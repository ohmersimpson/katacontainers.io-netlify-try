  ...
  render() {
    ...
    return (        
        ...
        :
          posts.map(({ node: post }) => {
            if(customFilter) {
              if(post.frontmatter.author.name === customFilter || post.frontmatter.categories.find(e => e === customFilter)) {
                return (            
                  <div className="article-excerpt" metalink="https://www.google.com/" key={post.id}>
                    ...
                    <div className="article-excerpt-meta">
                      <p>By <Link to={`/author/${post.frontmatter.author.slug}/`}>{post.frontmatter.author.name}</Link> on {post.frontmatter.date}</p>                
                    </div>
                  </div>
                )
              } else {
